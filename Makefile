...
PACKER_AMD64_IMAGE_NAME = local/packer:$(PACKER_AMD64_VERSION)

build-packer-amd64:
	docker build -f $(PACKER_AMD64_DIR)/Dockerfile \
	             -t $(PACKER_AMD64_IMAGE_NAME) \
...
PACKER_ARM_IMAGE_NAME = local/packer:$(PACKER_ARM_VERSION)

build-packer-arm:
	docker build -f $(PACKER_ARM_DIR)/Dockerfile \
	             -t $(PACKER_ARM_IMAGE_NAME) \
